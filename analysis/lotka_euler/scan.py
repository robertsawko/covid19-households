'''Run and pickle the result of a scan along the following model parameters:
    - global reduction 
    - compliance
'''
from copy import deepcopy
from numpy import array, linspace
from dill import dump
from models.utils import from_dict
from models.configs import CONFIG_WITH_DOUBLING_TIME as SPEC

if __name__ == '__main__':
    comply_range = linspace(0.0, 1.0, 4)
    globred_range = array([0.0, 0.5, 1.0])
    npi_types = ['individual', 'weak']

    msg = 'Done global reduction range {0:d} of {1:d} and compliange range {2:d} of {3:d}'
    results = {npi: [] for npi in npi_types}

    for ig, g in enumerate(globred_range):
        partial_results = {npi: [] for npi in npi_types}
        for ic, c in enumerate(comply_range):
            for npi in npi_types:
                spec = deepcopy(SPEC)
                spec['npi']['type'] = npi
                spec['npi']['compliance'] = c
                spec['npi']['global_reduction'] = g
                m = from_dict(spec)
                m.solve()
                partial_results[npi].append(m)
            print(msg.format(
                ig + 1, len(globred_range), ic + 1, len(comply_range)))
        for npi in npi_types:
            results[npi].append(partial_results[npi])

    with open('le-outputs.pkl', 'wb') as f:
        dump(results, f)
